...
-- --- 8. MARK MANAGER ---
-- State für den Mark Manager (wird nicht persistiert zwischen Restarts, 
-- nutzt aber die persistenten Vim-Marks)
local mark_state = {
    local_order = {'a', 'b', 'c', 'd', 'e'},
    global_order = {'A', 'B', 'C', 'D', 'E'},
    pinned = {} -- Speichert welche Register geschützt sind: { a = true, A = true }
}
-- Hilfsfunktion: Schiebt ein Register ans Ende der Liste (als "neuestes")
local function touch_mark(name, is_global)
    local list = is_global and mark_state.global_order or mark_state.local_order
...
function M.set_mark_auto(is_global)
    ...
    for _, m in ipairs(list) do
        if not mark_state.pinned[m] then
            target = m
            break
        end
    end
    ...
    local function add_marks(mark_list, targets, is_local)
        for _, m in ipairs(mark_list) do
            ...
            if targets[clean_mark] then
                local lnum = m.pos[2]
                local is_pinned = mark_state.pinned[clean_mark]
                ...
                table.insert(items, string.format("%s %s │ %-20s │ %4d │ %s", clean_mark, pin_str, fname, lnum, vim.trim(content)))
            end
        end
    end
    ...
    M.open_picker(items, title, function(selected)
        ...
        if mark_name then
            ...
            if is_global then
                ...
                -- Zu lokalem Mark im richtigen Puffer springen
                vim.api.nvim_set_current_buf(origin_bufnr)
                vim.cmd("normal! '" .. mark_name)
            end
            touch_mark(mark_name, is_global)
        end
    end, {
        p = function(selected, p_bufnr)
            local mark_name = selected:match("^(.)")
            M.toggle_mark_pin(mark_name)
            local cursor = vim.api.nvim_win_get_cursor(0)
            vim.api.nvim_win_close(0, true)
            M.show_marks(origin_bufnr) -- Context erhalten!
        ...
        dd = function(selected, dd_bufnr, dd_winnr)
            ...
            if mark_name then
                local is_global = mark_name:match("%u") ~= nil
                -- 1. In Vim löschen (mit API statt cmd für Buffer-Sicherheit)
                if is_global then
                    vim.api.nvim_del_mark(mark_name)
                else
                    vim.api.nvim_buf_del_mark(origin_bufnr, mark_name)
                end
                -- 2. In Lua State bereinigen
                mark_state.pinned[mark_name] = nil
                -- 3. In der Rotation nach ganz vorne schieben
                local list = is_global and mark_state.global_order or mark_state.local_order
                for i, m in ipairs(list) do
                    if m == mark_name then
                        table.remove(list, i)
                ...
                -- 4. Zeile aus dem Buffer entfernen
